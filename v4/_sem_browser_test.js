// v4.9.0 浏览器实测（真实 UI 全链路）：上传逐字稿 → 选品 → 评分 → 语义升级 → 落库唯一性
// 用法：node v4/_sem_browser_test.js
const fs = require('fs');
...
// PISTACHIO 2 风格逐字稿（srt）：覆盖 参数/材质/轮子/抗菌/前开盖/收纳/售后/场景CTA + 一处否定式场景表达
const SRT = `1
...
(async () => {
  ...
  await page.evaluate(() => {
    ...
    localStorage.removeItem('grading_history_v1');   // 清历史 → 数条数验证唯一性
  });
  await page.reload({ waitUntil: 'load' }).catch(() => {});
  await page.waitForTimeout(2000);

  // 1) 上传逐字稿 + 选品（表单可能在隐藏 tab → JS 直设，绕开可见性检查）
  //    文件名合规格式：主播_日期_直播间_原文 → autoDetectMeta 解析出 host/date/studio 才落库
  ...
  // 2) 触发评分（直接调 run()，与 #runBtn onclick 相同）
  ...
  console.log('[1] 已触发评分，等待语义判定…（DeepSeek 需 ~20s）');

  // 3) 轮询等 #semStatus 出现完成/降级文本（最多 90s）
  let status = '';
  const t0 = Date.now();
  ...
  const result = await page.evaluate(() => {
    ...
    const totalEl = resultBox ? resultBox.querySelector('.total, [class*=total]') : null;
    return {
      status: st ? st.textContent : '(无状态条)',
      histCount: Array.isArray(hist) ? hist.length : 'parse-fail',
      histLast: Array.isArray(hist) && hist.length ? JSON.stringify(hist[hist.length - 1]).slice(0, 300) : '',
  ...
  console.log('历史记录数:', result.histCount, '(应为 1，验证语义版只落一次库)');
  if (result.histLast) console.log('最近一条:', result.histLast);
  console.log('页面错误(' + errors.length + '):');
  errors.slice(0, 6).forEach(e => console.log('  ' + e));

  await page.screenshot({ path: path.join(__dirname, '_sem_shot_result.png'), fullPage: true });

  // 切到「每日评分」tab 截第二张（含 #result + 状态条 + 语义分）
  await page.evaluate(() => { location.hash = '#/daily'; });
  await page.waitForTimeout(1500);
  await page.screenshot({ path: path.join(__dirname, '_sem_shot_daily.png'), fullPage: true });
  console.log('截图2已存 v4/_sem_shot_daily.png（每日评分 tab · 含状态条）');

  // 再切到「历史评分」tab 看历史记录
  await page.evaluate(() => { location.hash = '#/history'; });
  await page.waitForTimeout(1500);
  await page.screenshot({ path: path.join(__dirname, '_sem_shot_history.png'), fullPage: true });
  console.log('截图3已存 v4/_sem_shot_history.png（历史评分 tab · 验证落库可见）');
