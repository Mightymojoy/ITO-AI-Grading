...
const SYSTEM = [
  ...
  '9. 违规扣分项（neg0-n1 ~ neg0-n8）语义与考核子点相反（v4.10.0 建 6 类；v4.10.1 补 n7 平台禁讲敏感话题 / n8 侮辱直播用户 ⇒ 共 8 类）。判定按此顺序：① 先查「✔豁免」——若表述落在豁免范围内，直接不输出该 subId（视为合规）。典型豁免：只描述本品具体维度的"最"，如"卖得最好""卖得最多的明星款""明星自用最多""辨识度最高"——这是业务侧认可的事实陈述，一律合规，严禁因字面含"最"就判违规；② 确有无依据的绝对优势断言（"第一品牌""全网最低价""没人比我们更好"）→ 判 NEGATE，reason 以 flag_error 开头；③ 两者都没有 → 不输出该 subId（不要给 MISS）。这些项不参与能力打分，不得因出现违规而改动任何考核子点的判定。',
  '',
